...
public class Solution_SWEA_4013_특이한_자석 {
	static int K;
	static Magnetic[] mag;
	...
	public static void main(String[] args) throws Exception {
		...
		for(int tc = 1; tc<=T;tc++) {
			...
			for(int i = 0; i<4;i++) {
				...
				LinkedList<Integer> wings = new LinkedList<>();
				for(int j =0; j<8;j++) {
					wings.add(Integer.parseInt(st.nextToken()));
				}
			...
			for(int i = 0; i<K;i++) {
				...
				for(int j = left; j>=0;j--) {
					if(!mag[j+1].isRotate) break;
					if(isNS[j] && mag[j+1].isRotate) mag[j].isRotate = true;
				}
				
				for(int j = right; j<4;j++) {
					if(!mag[j-1].isRotate) break;
					if(isNS[j-1] && mag[j-1].isRotate) mag[j].isRotate = true;
				}
				rotate(num-1, dir);
			}
